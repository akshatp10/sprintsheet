...
interface ProjectStageProps extends React.HTMLAttributes<HTMLDivElement> { label: string; color: string; tag: string | null; }

const ProjectStage = ({ color, label, tag, className, ...props }: ProjectStageProps) => {
    return (
        <div
            className={cn(
                "flex items-center justify-between rounded-md border border-lines-hairline w-full px-3 py-2",
                className,
            )}
            {...props}
        >
